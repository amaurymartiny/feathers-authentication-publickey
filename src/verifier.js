...
class PublicKeyVerifier {
  ...
  _verifySignature(entity, signature) {
    if (!entity[this.options.nonceField]) {
      return Promise.reject(
        new errors.BadRequest(
          `'${this.options.entity}' record in the database is missing a '${
            this.options.nonceField
          }' field`
        )
      );
    }
    ...
    const web3 = new Web3();
    const hashedNonce = utils.hashPersonalMessage(
      utils.toBuffer(entity.nonce.toString())
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      if (adr === entity[this.options.publicKeyField].toLowerCase()) {
        ...
        return reject(
          new errors.NotAuthenticated('Signature verification failed')
        );
      }
    });
  }
    ...
    this.service
      ...
      .catch(
        error =>
          error
            ? done(error)
            : done(null, error, { message: 'Signature verification failed' })
      );
  }
}
